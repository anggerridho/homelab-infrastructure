...
${HOMELAB}/terminate.sh -t 3 sshpass -p "$(echo ${MIKROTIK_PSSWD} | base64 -d)" ssh -t -oStrictHostKeyChecking=no ${MIKROTIK_HOST} "/system backup save name=backup_$DATE.backup; /export file=backup_$DATE.rsc"
...
${HOMELAB}/terminate.sh -t 3 sshpass -p "$(echo ${MIKROTIK_PSSWD} | base64 -d)" ssh -t -oStrictHostKeyChecking=no ${MIKROTIK_HOST} "/file remove [find name=backup_$DATE.backup]; /file remove [find name=backup_$DATE.rsc]"
echo "\n"
