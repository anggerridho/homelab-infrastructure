...
PART_500GB=$(lsblk -p -l -n -o NAME,SIZE,TYPE | awk '$3=="part" && $2 ~ /^465/ {print $1}' | head -n 1)
...
# ==========================================
# 2. Mount size ~465G ke /mnt/disk1
# ==========================================
if [ -n "$PART_500GB" ]; then
    echo "[OK] Ditemukan partisi ~465G di: $PART_500GB"
    mkdir -p /mnt/disk1
    
    if grep -qs '/mnt/disk1 ' /proc/mounts; then
        echo " -> /mnt/disk1 sudah ter-mount sebelumnya. Melewati..."
    else
        mount "$PART_500GB" /mnt/disk1
        if [ $? -eq 0 ]; then
            echo " -> Berhasil mount $PART_500GB ke /mnt/disk1"
        else
            echo " -> [ERROR] Gagal mount $PART_500GB ke /mnt/disk1"
        fi
    fi
else
    echo "[FAIL] Partisi dengan ukuran ~465G tidak ditemukan!"
fi
